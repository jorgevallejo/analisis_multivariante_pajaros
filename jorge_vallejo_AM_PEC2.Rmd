...
# Ejercicio 1. Los pájaros de Hunt
...
## 1j. De los apartados anteriores se deduce que hay un número razonable de conglomerados, aunque no sea óptimo. Dibujar el dendograma del apartado (g) con esa partición.
...
# Ejercicio 2. Los salmones de Johnson

**Un estudio contiene dos medidas de los anillos de crecimiento en la escala del salmón de Alaska y de Canadá. Los datos se pueden obtener 3 en el libro de Johnson et al. [-@johnson] y se adjuntan en el archivo `salmon.txt`.**

En primer lugar, como preparación al ejercicio cargaremos los datos a memoria y examinaremos las características y estructura de los mismos.

```{r}
salmon <- read.table("./data/salmon.txt",
                     header = TRUE)

kable(
  head(salmon),
  caption = "Ejemplo de las primeras seis observaciones en la base de datos `salmon.txt`."
)
```
...
La variable `Gender` la codificaremos como factor, no como numérica:
```{r}
salmon$Gender <- factor(salmon$Gender, labels = c("female", "male"))
```

La base de datos consta de 100 observaciones (filas) y 4 variables (columnas). De las cuatro variables, dos son categóricas ( _Gender_ y _Origin_) y dos son contínuas ( _Freshwater_ y _Origin_ ).

**Gender:** hembra = 1 y macho = 2.  
**Freshwater**: Diámetro de los anillos del primer año de crecimiento en agua dulce (centésimas de pulgada).  
**Marine:** Diámetro de los anillos del primer año de crecimiento en ambiente marino (centésimas de pulgada).  
**Origin:** Origen del pez. 1 = Alaskan, 2 = Canadian.  
...
No parece haber valores anómalos (entendidos como imposiblemente elevados, negativos, o faltantes) en ninguna de las variables. Las variables categóricas están ambas equilibradas, especialmente _Origin_.

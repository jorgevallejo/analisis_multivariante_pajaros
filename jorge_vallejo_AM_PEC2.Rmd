...
for (i in 1:(ncols-1)){
  for (j in i:ncols){
    D2_chisq[i,j] <- as.matrix((table_Pc[i, ] - table_Pc[j, ])) %*%
       diag(1 / colSums(table_F)) %*% # Diagonal matrix from column margin
      t(table_Pc[i, ] - table_Pc[j, ])
  }
}
# Names for distance matrix
rownames(D2_chisq) <- colnames(D2_chisq) <- rownames(table_N)
